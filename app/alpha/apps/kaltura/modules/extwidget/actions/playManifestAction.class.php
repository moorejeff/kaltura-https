...
class playManifestAction extends kalturaAction
{
	...
	private function getFlavorHttpUrl(flavorAsset $flavorAsset)
	{
		...
		return KALTURA_HTTP_PROTOCOL . '://' . $url;
	...
	private function serveMediaFile()
	{
		if($this->entry->getType() != entry::ENTRY_TYPE_MEDIACLIP)
			KExternalErrors::dieError(KExternalErrors::INVALID_ENTRY_TYPE);
		...
		switch($this->entry->getType())
		{
			case entry::ENTRY_TYPE_MEDIACLIP:
				switch($this->entry->getMediaType())
				{
					...
					case entry::ENTRY_MEDIA_TYPE_VIDEO:
					case entry::ENTRY_MEDIA_TYPE_AUDIO:	
						$fauxfilename = preg_replace('/^.+\/([^\/]+)$/', "$1", $_SERVER['SCRIPT_URL']); 
						$flavors = $this->buildFlavorsArray($duration, true);
						$mediapath = preg_replace('/^(.+\/)seekFrom\/.*$/', "$1", $flavors[0]['url']);
						$mediaurl = $mediapath . $fauxfilename;
...
/*  Modified by AR per http://www.kaltura.org/kaltura-ce-v30-red5-integration-guide
					foreach($flavorAssets as $flavorAsset)
					{
...
*/
			...
			case entry::ENTRY_TYPE_LIVE_STREAM:
				...
				$baseUrl = "rtmp://kaltura-test.swarthmore.edu/oflaDemo";   //"rtmp://cp{$streamUsername}.live.edgefcs.net/live";
	...
	public function execute()
	{
		...
		switch($this->format)
		{
			case StorageProfile::PLAY_FORMAT_HTTP:
				$xml = $this->serveHttp();
				break;
			//added by les to automatically load the media file for HTML5 use
			case 'url':
				$this->serveMediaFile();
				break;
